...
export class UI {
  ...
  update(state: Snapshot, p: Player, ping: number): void {
    ...
    this.text('hud-job', p.jobTitle ?? job.name);
    this.el('hud-job').title = job.name;
  ...
  renderMenu(): void {
    ...
    if (this.menu === 'jobs' && p)
      html += `<section class="job-title-editor"><h3>Your roleplay title</h3><p class="muted">Name your business or specialty. Your actual role remains ${JOBS[p.job].name}, with the same salary and permissions. Changing jobs resets the title.</p><div class="command-field"><input id="job-title" maxlength="32" value="${escape(p.jobTitle ?? '')}" placeholder="${escape(JOBS[p.job].name)}" aria-label="Roleplay job title"><button data-action="job-title" ${p.deadUntil || p.arrestedUntil ? 'disabled' : ''}>Save title</button><button data-action="job-title-reset" ${p.deadUntil || p.arrestedUntil ? 'disabled' : ''}>Reset</button></div></section>`;
  ...
  residentHtml(id: string): string {
    ...
    return `<div class="section-heading"><span class="eyebrow">RESIDENT</span><h2>${escape(target.name)}</h2><p><span style="color:${JOBS[target.job].color}">${target.jobTitle ? `${escape(target.jobTitle)} · ` : ''}${JOBS[target.job].name}</span> · ${target.deadUntil ? 'Respawning' : target.arrestedUntil ? 'In custody' : 'In the district'}</p></div>
  ...
  clickAction(action: string, target: string, value?: string): void {
    ...
    if (action === 'job-title' || action === 'job-title-reset') {
      this.onAction(
        'job-title',
        undefined,
