...
OpenRP is a playable **0.1 alpha**, not a complete Source engine port. All map geometry, characters, equipment, textures, and sounds are original and generated locally. No Garry’s Mod installation, extracted Valve assets, external asset CDN, API keys, or paid services are required.
...
For internet play, run the Node server on a host that supports long-lived WebSockets and put HTTPS in front of it. Forward `/ws` upgrades to the same server as the page. Static hosts such as GitHub Pages alone cannot run the multiplayer server. See [hosting and configuration](docs/HOSTING.md).
