...
export class Game {
  ...
  residentAction(p: Player, action: ResidentAction, targetId: string, value?: unknown): void {
    ...
    if (action === 'wanted') {
      ...
      if (target.license) return;
      target.license = true;
      this.notice(target.id, 'The mayor granted you a gun license.', 'success');
      this.notice(p.id, `Gun license granted to ${target.name}.`, 'success');
    }
  }
  command(p: Player, command: string, words: string[]): void {
    ...
    if (['/wanted', '/unwanted', '/warrant', '/license'].includes(command)) {
      ...
      this.residentAction(p, command.slice(1) as ResidentAction, target.id, reason);
      return;
    }
